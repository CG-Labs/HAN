...
all_embeddings.append(logits.numpy())
all_labels.append(batch_labels.numpy())
# Log the size of all_embeddings and all_labels to confirm they are being populated
logging.debug("all_embeddings size after append: %d", len(all_embeddings))
logging.debug("all_labels size after append: %d", len(all_labels))
jhy_final_embedding = np.concatenate(all_embeddings, axis=0)
